...
import { cotizaciones, dashboardStats, reencauchesPorMes, salesByBrand } from '../../../data/mockData';

/**
 * Dashboard del administrador.
 *
 * Solo lectura: dos indicadores del mes, los reencauches procesados, la
 * participación por marca y los últimos pedidos-cotización registrados.
 */
    ...
    <div>
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          ...
          <button className="ml-pulsable inline-flex items-center gap-2 rounded-lg bg-amber-400 px-3.5 py-2 text-sm font-semibold text-slate-900 hover:bg-amber-300">
      ...
      <div className="mt-4 grid grid-cols-1 gap-4 xl:grid-cols-[1.6fr_1fr]">
        {/* ---- Reencauches procesados por mes ---- */}
      ...
      <div className="mt-4 rounded-xl border border-slate-200 bg-white p-5 dark:border-white/10 dark:bg-brand-navy-800">
        ...
        <ul className="mt-3 divide-y divide-slate-100 dark:divide-white/5">
          {cotizaciones.map((pedido) => (
            <li key={pedido.id} className="flex flex-wrap items-center justify-between gap-2 py-3">
              <div>
                <p className="text-sm font-semibold text-slate-800 dark:text-slate-100">{pedido.id}</p>
                <p className="text-xs text-slate-500 dark:text-slate-400">{pedido.cliente}</p>
              </div>
              <div className="flex items-center gap-3">
                <span className="font-mono text-sm text-slate-700 dark:text-slate-200">{pedido.total}</span>
